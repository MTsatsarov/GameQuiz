...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20211127142416_smth')
BEGIN
...
BEGIN
    CREATE TABLE [ApplicationUserQuiz] (
        ...
        CONSTRAINT [FK_ApplicationUserQuiz_AspNetUsers_UsersId] FOREIGN KEY ([UsersId]) REFERENCES [AspNetUsers] ([Id]) ON DELETE CASCADE,
        CONSTRAINT [FK_ApplicationUserQuiz_Quizzes_QuizzesId] FOREIGN KEY ([QuizzesId]) REFERENCES [Quizzes] ([Id]) ON DELETE CASCADE
    );
END;
GO
...
BEGIN
    CREATE TABLE [Votes] (
        ...
        CONSTRAINT [FK_Votes_Quizzes_QuizId] FOREIGN KEY ([QuizId]) REFERENCES [Quizzes] ([Id]) ON DELETE CASCADE
    );
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20211127142416_smth')
BEGIN
    CREATE TABLE [ApplicationUserVote] (
        [UsersId] nvarchar(450) NOT NULL,
        [VotesId] int NOT NULL,
        CONSTRAINT [PK_ApplicationUserVote] PRIMARY KEY ([UsersId], [VotesId]),
        CONSTRAINT [FK_ApplicationUserVote_AspNetUsers_UsersId] FOREIGN KEY ([UsersId]) REFERENCES [AspNetUsers] ([Id]) ON DELETE CASCADE,
        CONSTRAINT [FK_ApplicationUserVote_Votes_VotesId] FOREIGN KEY ([VotesId]) REFERENCES [Votes] ([Id]) ON DELETE CASCADE
    );
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20211127142416_smth')
BEGIN
    CREATE INDEX [IX_ApplicationUserVote_VotesId] ON [ApplicationUserVote] ([VotesId]);
END;
GO
...
BEGIN
    ...
    VALUES (N'20211127142416_smth', N'5.0.12');
END;
GO
